...
if ( ! file.exists("master_cor20240210.rds")){
  ...
  master_distance_MG<-master_dist_MG%>%group_by(Track2, mouse, position, class, Track2)%>%summarize(n_MG=max(n_MG), min_MG =min(min_MG))
  ### plot per position types the values of 
  ...
  saveRDS(master_cor,"master_cor20240210.rds")
  saveRDS(master_distance_MG,"master_distance_MG20240210.rds")
  saveRDS(master_distance_SR101,"master_distance_SR10120240210.rds")
  saveRDS(BV_df_sum,"BV_df_sum20240210.rds")
}else{
  master_cor<-readRDS("master_cor20240210.rds")
  master_distance_MG<-readRDS("master_distance_MG20240210.rds")
  master_distance_SR101<-readRDS("master_distance_SR10120240210.rds")
  BV_df_sum<-readRDS("BV_df_sum20240210.rds")
...
master_cor2<-master_cor2 %>% 
  group_by(subsequence)%>%filter(Time2<2.6)  ##Minimal time for a position is 
...
subtrack_length<-master_cor2%>%group_by(Track2, position, subsequence)%>%
  summarise(n=n_distinct(Time2), first_t=min(Time2), last_t=max(Time2))%>%
  ungroup()
hist(subtrack_length$n)
subtrack_length<-subtrack_length%>%filter(n==13)  # we need to automate this to get the most frequent n
#Now we only keep these subtracks that have the same length:
master_cor2<-subset(master_cor2, subsequence%in%subtrack_length$subsequence)
...
##number of unique subtracks that are processed:
length(unique(master_cor2$subsequence))
...
# Calculate displacement square for each time interval within each subsequence group
master_cor2 <- master_cor2 %>%
  group_by(subsequence) %>%
  arrange(Time2) %>%
  mutate(Displacement_X = c(0, diff(x - first(x))),
         Displacement_Y = c(0, diff(y - first(y))),
         Displacement_Z = c(0, diff(z - first(z))),
         Squared_Displacement = Displacement_X^2 + Displacement_Y^2 + Displacement_Z^2,
         disp2 = cumsum(Squared_Displacement)) %>%
  select(-Displacement_X, -Displacement_Y, -Displacement_Z, -Squared_Displacement) %>%
  ungroup()
# Normalize disp_l and disp_d to the first timepoint
master_cor2 <- master_cor2 %>%
  group_by(subsequence) %>%
  arrange(Time2) %>%
  mutate(Displacement_X = x - first(x),
         Displacement_Y = y - first(y),
         Displacement_Z = z - first(z),
         disp_l= sqrt(Displacement_X^2 + Displacement_Y^2 + Displacement_Z^2)) %>%
  select(-Displacement_X, -Displacement_Y, -Displacement_Z)%>%
  ungroup()
...
column_names2<-subset(column_names2,!column_names2 %in%c("Time","Track2","subsequence","x","y","z","position","mouse","class","Time2", "dist_tumor_1", "dist_3_neigh", "dist_10_neigh", "iteration", "Track2"))
...
###Check differences between subsequences at different timepoints:

master_pca3_test<-left_join(master_pca3,master_cor2)
master_pca3_sum<-master_pca3_test%>%group_by(subsequence)%>%arrange(Time)%>%
  summarise(Time=first(Time), disp2_mean=mean(disp2), mean_PC1=mean(PC1), mean_PC2=mean(PC2), mean_PC3=mean(PC3),
            disp2_min=quantile(disp2,0.10), disp2_max=quantile(disp2, 0.9), mean_speed=mean(speed), min_speed=quantile(speed, 0.10),
            max_speed=quantile(speed, 0.90))

ggplot(subset(master_pca3_sum, Time%in%c(0,1.4,2.6)),aes(x=as.factor(Time) , y=max_speed)) +
  geom_boxplot(outlier.colour = NA, alpha=0.5)+theme_classic()+
  theme(aspect.ratio=0.7)+coord_cartesian(ylim = c(0,100))
...
if ( ! file.exists("matrix_distmat_20240209_n_6_IVM_subsequence_pca3.rds")){
  ##parallel working
  # load parallel
  library(parallel)
  ...
  saveRDS(distmat, file = "distmat_20240209_n_6_IVM_subsequence_pca3.rds")
  
  matrix_distmat<-as.matrix(distmat)
  saveRDS(matrix_distmat, file = "matrix_distmat_20240209_n_6_IVM_subsequence_pca3.rds")
  

}else{
  matrix_distmat <- readRDS("matrix_distmat_20240207_n_6_IVM_subsequence_pca3.rds")
  }
...
umap_dist<- umap(matrix_distmat,n_components=2,input="dist",init = "random", 
                 n_neighbors=20,min_dist=0.4, spread=4,n_epochs=1500 , local_connectivity=5) ## here there should be some parameters to play with to adjust according number of cells etc. check behav3D github for more information 
#3D plot if necessary:12 / 0.06 /3//  12 /0.08/3    /15/0.8/5  //15/0.7/5  //15 / 0.4/5
#n_neighbors=15,min_dist=0.4, spread=4,n_epochs=1000 , local_connectivity=5
#n_neighbors=20,min_dist=0.4, spread=4,n_epochs=1000 , local_connectivity=5
#n_neighbors=20,min_dist=0.4, spread=4,n_epochs=3000 , local_connectivity=5
#n_neighbors=20,min_dist=0.4, spread=4,n_epochs=2000 , local_connectivity=5 >7cl
#n_neighbors=20,min_dist=0.4, spread=4,n_epochs=1500 , local_connectivity=5 >8 cl
...
positiontype<-master_norm[,c("subsequence", "position","class","mouse")]
...
hierarchical_clusters <- cutree(hc, k = 8) 
...
mypalette_1<- continuous_palette[seq(1, length(continuous_palette), length.out = 8)]
...
res_path<-"E:/SURF_2/Intravital imaging/3.Analysis/Results_IVM_paper_preparation/" 
...
master_class <- left_join(master_cor2 ,umap_3[c(1:4)], by.x = c("subsequence"), by.y = c("subsequence"))
master_class<-master_class %>% 
  filter(cluster2 != 0)
master_class$cluster2<-as.numeric(master_class$cluster2)
master_class_sum<-master_class%>%group_by(subsequence,position,class,mouse, Track2)%>%
...
pdf(paste0(res_path, "cluster_heatmap_dynamic_feaures_2.pdf"))
...
pheatmap(df_cl_features, clustering_method = "complete", cluster_cols=F,cluster_rows=F,cellwidth=20, cellheight=20,
                 treeheight_row = 0, treeheight_col = 0,fontsize = 8,na_col = "grey50", main="features used for clsutering",
                 angle_col = 90,color = colorRampPalette(c("deepskyblue1", "grey95", "deeppink2"))(50))
...
pheatmap(df_other_features, clustering_method = "complete", cluster_cols=F,cluster_rows=F,cellwidth=20, cellheight=20,
         treeheight_row = 0, treeheight_col = 0,fontsize = 8,na_col = "grey50",main="features not used for clustering",
         angle_col = 90,color = colorRampPalette(c("deepskyblue1", "grey95", "deeppink2"))(50))
...
master_class_sum$cluster1<-NA
master_class_sum$cluster1<-ifelse(master_class_sum$cluster2%in%c(2), "invading_1",master_class_sum$cluster1)
master_class_sum$cluster1<-ifelse(master_class_sum$cluster2%in%c(6), "invading_2",master_class_sum$cluster1)
master_class_sum$cluster1<-ifelse(master_class_sum$cluster2%in%c(3), "retreating_1",master_class_sum$cluster1)
master_class_sum$cluster1<-ifelse(master_class_sum$cluster2%in%c(1), "fast",master_class_sum$cluster1)
master_class_sum$cluster1<-ifelse(master_class_sum$cluster2%in%c(4), "slow_persistant",master_class_sum$cluster1)
master_class_sum$cluster1<-ifelse(master_class_sum$cluster2%in%c(7), "erratic",master_class_sum$cluster1)
master_class_sum$cluster1<-ifelse(master_class_sum$cluster2%in%c(5,8), "slow",master_class_sum$cluster1)

pdf(paste0(res_path, "UMAP_first_level clusters.pdf"))

ggplot(master_class_sum, aes(x = V1, y = V2, color = as.factor(cluster1))) +  
  geom_point(size = 2, alpha = 1) + 
  labs(color = "Cluster") +
  xlab("") + 
  ylab("") +  # Match palette to cluster2 levels  ggtitle("UMAP Cluster") +
  theme_light(base_size = 20) +
  theme(axis.text.x = element_blank(),
...
pdf(paste0(res_path, "UMAP_cluseter_and_other features.pdf"))
...
p_class_speed <- ggplot(master_class_sum2,aes(x=as.factor(class) , y=speed, group=class,fill=class)) +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5)+scale_fill_manual(values=c("cyan","gold","red"))+ggtitle("speed per env cluster")+theme_classic()+
  theme(aspect.ratio=2)+coord_cartesian(ylim = c(0,20))
...
p_class_movement <- ggplot(master_class_sum2,aes(x=as.factor(class) , y=movement, group=class,fill=class)) +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5)+scale_fill_manual(values=c("cyan","gold","red")) +ggtitle("movement direction per env cluster")+theme_classic()+
  theme(aspect.ratio=2)+scale_color_gradientn(colours = rev(RColorBrewer::brewer.pal(4, "Spectral")))+coord_cartesian(ylim = c(-10,17))
...
p_class_dist_10neigh <- ggplot(master_class_sum2,aes(x=as.factor(class) , y=dist_10_neigh, fill=class)) +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5)+scale_fill_manual(values=c("cyan","gold","red"))  +ggtitle("dist 10 neigh per cluster")+theme_classic()+
  theme(aspect.ratio=2)+coord_cartesian(ylim = c(20,70))
...
p_class_minMG <- ggplot(subset(master_class_sum2, !is.na(min_MG)) ,aes(x=as.factor(class) , y=min_MG, fill=class))  +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5)+scale_fill_manual(values=c("cyan","gold","red"))  +ggtitle("distance to closest MG")+theme_classic()+
  theme(aspect.ratio=2)+coord_cartesian(ylim = c(0,50))
...
p_classmin_SR101 <- ggplot(subset(master_class_sum2, !is.na(min_SR101)) ,aes(x=as.factor(class) , y=min_SR101, fill=class))  +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5) +scale_fill_manual(values=c("cyan","gold","red"))+ggtitle("distance to closest SR101")+theme_classic()+
  theme(aspect.ratio=2)+coord_cartesian(ylim = c(0,50))
...
p_class_n_MG <- ggplot(subset(master_class_sum2, !is.na(min_MG)),aes(x=as.factor(class) , y=n_MG, fill=class)) +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5) +scale_fill_manual(values=c("cyan","gold","red"))+ggtitle("n of MG per cluster")+theme_classic()+
...
p_classn_SR101 <- ggplot(subset(master_class_sum2, !is.na(min_SR101)),aes(x=as.factor(class) , y=n_SR101, fill=class)) +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5) +scale_fill_manual(values=c("cyan","gold","red"))+ggtitle("n SR101 per cluster")+theme_classic()+
  theme(aspect.ratio=2)
p_classn_SR101
...
p_class_BV_min <- ggplot(subset(master_class_sum2, !is.na(BV_min)) ,aes(x=as.factor(class) , y=BV_min, fill=class))  +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5)+scale_fill_manual(values=c("cyan","gold","red"))  +ggtitle("min distance to BV")+theme_classic()+
  theme(aspect.ratio=2)+coord_cartesian(ylim = c(0,18))
...
p_class_BV_mean <- ggplot(subset(master_class_sum2, !is.na(BV_mean)) ,aes(x=as.factor(class) , y=BV_mean, fill=class))  +
  geom_boxplot(alpha=0.5, outlier.colour = NA)+ geom_jitter(width = 0.2, alpha=0.5)+scale_fill_manual(values=c("cyan","gold","red"))  +ggtitle("mean distance to BV")+theme_classic()+
  theme(aspect.ratio=2)+coord_cartesian(ylim = c(0,18))
...
##Change of clusters overtime
library(dplyr)

set.seed(123)

# Assuming sankey_data contains the data
sankey_data<-master_class_sum2 %>% 
  group_by(Track2)%>%filter(n_distinct(subsequence)>1)%>%group_by(Track2)%>%arrange(Time)%>%mutate(Time = rank(Time))%>%ungroup()
#sankey_data$Time<-rank(sankey_data$Time)
sankey_data<-sankey_data%>%group_by(Time, cluster2, Track2)%>%summarise(n=n())%>%ungroup()
library(ggalluvial)
# Create alluvial plot

ggplot(sankey_data,
       aes(x = as.factor(Time), stratum = as.factor(cluster2), alluvium = Track2,
           y = n,
           fill = as.factor(cluster2), label = as.factor(cluster2))) +
  scale_x_discrete(expand = c(.1, .1)) +
  geom_flow() +
  geom_stratum(alpha = .5) +
  geom_text(stat = "stratum", size = 3) + scale_fill_manual(values=mypalette_1)+
  theme(legend.position = "none") +
  ggtitle("cluster change in time")+theme_classic()


#show only the change for two timepoints
sankey_data_t2<-sankey_data%>%group_by(Track2)%>%filter(n() >= 2 &Time<3)%>%ungroup()
ggplot(sankey_data_t2,
       aes(x = as.factor(Time), stratum = as.factor(cluster2), alluvium = Track2,
           y = n,
           fill = as.factor(cluster2), label = as.factor(cluster2))) +
  scale_x_discrete(expand = c(.1, .1)) +
  ...
  theme(legend.position = "none") +
  ggtitle("cluster change in time")




#show only the change for two timepoints
sankey_data_t3<-sankey_data%>%group_by(Track2)%>%filter(n() >= 3)%>%ungroup()
ggplot(sankey_data_t3,
       aes(x = as.factor(Time), stratum = as.factor(cluster2), alluvium = Track2,
           y = n,
           fill = as.factor(cluster2), label = as.factor(cluster2))) +
  scale_x_discrete(expand = c(.1, .1)) +
...
###plot all the tracks
Tracks_1<-Tracks_1[!duplicated(Tracks_1$subsequence),c("cluster2", "subsequence")]
...
first_time<-master_traject%>%group_by(Track2)%>%arrange(Time)%>%slice_head()%>%ungroup()
g<-ggplot(subset(master_traject,  position=="2430F11.CL2_1"))+geom_path(aes(x_new, y_new, group=Track2, 
                                        color=as.factor(cluster2)), size=0.2,
                                    arrow = arrow(ends = "last",type = "open",length = unit(0.01, "inches")))+
  geom_point(data = subset(first_time,  position=="2430F11.CL2_1"), aes(x = x_new, y = y_new), color = "grey50", size = 0.5) + # Add dot at the beginning
